...
class UserManager extends Module {
    ...
    constructor(options) {
        ...
        this.io.on('connection', socket => {
            ...
            socket.on(this.MESSAGES.TIME_GAMER, token => this.timeGamer(token, socket));
    ...
    async timeGamer(token, socket) {
        const time = await this.db.timeGamer(token);
        socket.emit(this.MESSAGES.TIME_GAMER, time);
    }
}
